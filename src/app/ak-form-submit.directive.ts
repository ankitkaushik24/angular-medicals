...
@Directive({
  selector: '[akFormSubmit]'
})
export class AkFormSubmitDirective implements OnInit {

  @Input('akFormSubmit') rootForm: FormGroup;
  ...
  @HostListener('click', ['$event']) onSubmit(event: MouseEvent) {
    if (this.rootForm.invalid) {
      ...
      const invalidEls: HTMLCollection = formContainer.getElementsByClassName('ak-control ng-invalid');
